...
$links = [
    ...
    [
        'name' => 'Khu trọ & Phòng',
        'route' => 'admin.houses.index', // Đã cập nhật
    ...
    [
        'name' => 'Khách thuê',
        'route' => 'admin.tenants.index', // Đã cập nhật
    ...
    [
        ...
        'route' => 'admin.meter-readings.index',
        'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>'
    ],
    [
        'name' => 'Hóa đơn & Thu tiền',
...
@foreach($links as $link)
    <a href="{{ route($link['route']) }}" class="group flex items-center px-3 py-3 text-sm font-medium rounded-xl transition-all duration-200 
        {{ request()->routeIs($link['route']) ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white' }}">
        
        <svg class="mr-3 flex-shrink-0 h-6 w-6 
            {{ request()->routeIs($link['route']) ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 group-hover:text-gray-500 dark:text-gray-500 dark:group-hover:text-gray-300' }}" 
